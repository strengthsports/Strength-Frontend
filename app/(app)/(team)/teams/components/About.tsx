...
const About: React.FC<AboutProps> = ({ teamDetails }) => {
  ...
  const handleButtonPress = () => {
    ...
    setIsSupporting((prevIsSupporting) => {
      ...
      setSupportersCount((prevCount) => prevIsSupporting ? prevCount - 1 : prevCount + 1);
    ...
    <ScrollView>
      <View className="p-4 bg-black rounded-lg">
        <View className="flex flex-row items-center justify-between">
          <View className="flex flex-row items-center justify-between">
            <Supporters />
            <Text className="text-white text-3xl ml-1 font-bold">{supportersCount}</Text>
            <Text className="text-gray-500 text-4xl font-bold "> Supporters</Text>
          </View>
          <CustomButton
        ...
        <Text className="text-white pt-8 ml-1 text-6xl font-bold mb-2">Description</Text>
        <Text className="text-white ml-1 text-lg" style={{ fontFamily: 'Sansation-Regular' }}>
          {teamDetails}
        </Text>
      </View>
      ...
      <View className="ml-4 mt-3 flex flex-row ">
        <EstabilishedOn />
        <Text className="text-white text-4xl mt-1 ml-1 ">Established on - April, 2024</Text>
      </View>

      <View className="ml-4 mt-3 flex flex-row ">
        <TeamId />
        <Text className="text-white text-4xl mt-1 ml-1 "> Team unique ID - RKR1421</Text>
        <CopyCode />
      </View>
    </ScrollView>
  );
