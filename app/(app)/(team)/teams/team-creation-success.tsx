...
const TeamCreatedPage: React.FC = () => {
  ...
  const teamData: TeamData = params.teamData
    ? JSON.parse(params.teamData as string)
    : null;
  ...
  // console.log("User Name------>", JSON.stringify(teamData.members, null, 2));
  ...
  return (
    <SafeAreaView className="flex-1 bg-black pt-2">
      <Animated.View style={{ flex: 1, transform: [{ translateY }] }}>
        ...
        <View className="flex flex-row justify-center  items-center py-8 w-full mt-8 mx-6 ">
          <Image
            className="w-32 h-32 mt-1 border-2 border-[#181818] rounded-[8px] ml-8"
            source={
              teamData?.logo
                ? { uri: teamData.logo.uri }
                : require("../../../../assets/images/teams/dummyteam.png")
            }
          />
          <Image
        ...
        <ScrollView className="flex-1 px-6 mt-3 ">
          {Array.from(
            new Map(
              teamData?.members?.map((member) => [member._id, member])
            ).values()
          ).map((member) => (
            <View key={member._id} className="flex-row items-center py-4">
              <Image
                source={{
                  uri: member.profilePic || "https://picsum.photos/200/200",
                }}
