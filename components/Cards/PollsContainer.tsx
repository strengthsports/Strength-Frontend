...
const PollsContainer: React.FC<PollsContainerProps> = memo(
  ({
    ...
    const [localVoteCounts, setLocalVoteCounts] = useState(() =>
      voteCounts && voteCounts.length === options.length
        ? [...voteCounts]
      ...
      >
        {(mode === "create" || onClose) && (
          <View style={styles.header}>
            <TextScallingFalse style={styles.title}>Poll</TextScallingFalse>
            {onClose && (
              <TouchableOpacity
                activeOpacity={0.5}
                ...
                {mode === "create" ? (
                  ...
                  >
                    <LinearGradient
                      ...
                      colors={
                        localUserVoted ? gradientColors : gradientDisabledColors
                      }
                      locations={
                        localUserVoted
                          ? gradientLocations
                          : gradientDisabledLocations
                      }
                      start={{ x: 0, y: 0.5 }}
                      end={{ x: 1, y: 0.5 }}
                    >
