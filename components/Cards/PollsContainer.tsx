...
const PollsContainer: React.FC<PollsContainerProps> = ({ onClose }) => {
  ...
  const addOption = () => {
    if (options.length < 5) { // Restrict to 5 options max
      setOptions([...options, ""]);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity activeOpacity={0.5} style={styles.closeButton} onPress={onClose}>
      ...
      {/* Add Option Button */}
      {options.length < 5 && (
        <TouchableOpacity activeOpacity={0.5} style={styles.addButton} onPress={addOption}>
