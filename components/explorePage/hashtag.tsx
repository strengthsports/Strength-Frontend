      ...
      {data.map((hashData) => (
        <View key={hashData.id}>
          <View className="px-5 flex-row">
            {/* Index */}
            <TextScallingFalse className="text-theme text-5xl font-bold mr-5">
              {hashData.id}
            </TextScallingFalse>

            {/* Hashtag and Posts Count */}
            <View className="flex-col justify-center">
              <TextScallingFalse className="text-[#CECECE] text-3xl font-bold ">
                {hashData.hashtag}
              </TextScallingFalse>
              <TextScallingFalse className="text-[#6E6E6E] text-2xl mt-1">
          ...
          {/* Divider */}
          <View className=" h-[0.6px] bg-[#474747] my-3 ml-14 mr-6" />
        </View>
      ))}
    </>
  );
