...
import { View, Image, TouchableOpacity, FlatList, ActivityIndicator, Modal, TextInput } from 'react-native';
...
const ReportModal = memo(({ commentId, targetId, targetType, showDelete }: ReportModalProps) => {
  ...
  const handleDeleteComment = async () => {
    try {
      ...
      const response = await deleteComment({ commentId,  targetId, targetType }).unwrap();
      ...
      <View className="h-28 w-[104%] bg-neutral-900 self-center rounded-t-[40px] p-4 border-t border-x border-neutral-700">
        ...
        <View className="flex-1 justify-evenly">
          ...
          (<TouchableOpacity
            ...
            disabled={isDeleting}
            >
            {isDeleting ? <ActivityIndicator color={Colors.themeColor} /> : <MaterialIcons name="delete" size={20} color="red" />}
            ...
            <TouchableOpacity className="flex-row items-center py-2 px-2 active:bg-neutral-800 rounded-lg">
              ...
              <TextScallingFalse className="text-white ml-4">Report</TextScallingFalse>
            </TouchableOpacity>
          )}
        </View>
      </View>
    ...
    <View className="pl-12 pr-1 py-2 my-2 relative ">
      ...
      <View className="relative w-full bg-neutral-900 rounded-xl py-2 px-10">
        <View className="absolute right-3 top-2 flex flex-row items-center gap-2 ">
          <TextScallingFalse className="text-xs text-neutral-300">1w </TextScallingFalse>
          <TouchableOpacity onPress={() => setIsReportModalVisible(true)}>                    
            ...
            {isReportModalVisible && (
              ...
              >
                ...
                <ReportModal commentId={comment._id}
                  targetId={targetId}
        ...
        <View>
          ...
          <TextScallingFalse className="text-xs w-4/5 text-neutral-200" numberOfLines={1} ellipsizeMode="tail">
            {comment?.commenter?.headline}
          </TextScallingFalse>
        </View>
        <TextScallingFalse className="text-xl text-white mt-4 mb-3">{comment?.text}</TextScallingFalse>
      </View>
      <View className="flex-row gap-2 items-center ml-10 mt-1">
        <TouchableOpacity>
          <TextScallingFalse className="text-white text-lg">Like</TextScallingFalse>
        </TouchableOpacity>
        <TextScallingFalse className="text-xs text-white">|</TextScallingFalse>
        <TouchableOpacity>
          <TextScallingFalse className="text-white text-lg">Reply</TextScallingFalse>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const CommentModal = memo(({ autoFocusKeyboard = false, targetId }: CommentModalProps) => {
    ...
    return (
        <View className="h-3/4 w-[104%] self-center bg-black rounded-t-[40px] p-4 border-t border-x border-neutral-700">
            ...
            <TextScallingFalse className="text-white self-center text-2xl my-4">Comments</TextScallingFalse>
            <View className="mb-32">
                {isFetching ? (
                    ...
                    <FlatList
                        ...
                        ListEmptyComponent={<TextScallingFalse className="text-white text-center">No Comments Found!</TextScallingFalse>}
                    />
                )}
            </View>
                ...
                <View className="bg-black p-2">
                    <View className="w-full self-center flex items-center flex-row justify-around rounded-full bg-neutral-900">
                        ...
                        <TouchableOpacity onPress={handlePostComment} disabled={isPosting}>
                            <MaterialIcons className='p-2' name="send" size={22} color={isPosting ? '#292A2D' : commentText? Colors.themeColor : 'grey'} />
                        </TouchableOpacity>
