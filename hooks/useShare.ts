...
export const useShare = () => {
 ...
 const shareArticles = useCallback(
  async ({
    ...
    content?: string;
    banner?: string;
    sportsName?: string; // single sport name
    date?: string;
  }) => {
    try {
      const formattedDate = date ? new Date(date).toDateString() : "Today";

      const message =
        `📰 *${title || "Sports Article"}*\n\n` +
        `${content ? content.substring(0, 200) + "..." : ""}\n\n` +
        `📅 Published on: ${formattedDate}\n` +
        `🏆 Sport: ${sportsName || "General"}\n\n` +
        `Read more ${sportsName || "General"} related news and stay updated with ${APP_NAME}!\n\n` +
        `Download the app: ${LINK}`;

      if (Platform.OS === "ios") {
        await Share.share({ message });
      } else if (Platform.OS === "android" && banner) {
        const fileUri = `${FileSystem.cacheDirectory}${Date.now()}.jpg`;
        await FileSystem.downloadAsync(banner, fileUri);
        await Share.share({
          message,
          url: fileUri,
        });
      ...
      console.error("Sharing article failed", err);
      showFeedback("Failed to share article.");
    }
  },
  []
);

  return { sharePost, shareProfile, shareArticles};
};
